...
| --------------------- | -------- | ---------- | -------- | -------- | ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| React app shell       | Yes      | Yes        | Yes      | Partial  | No               | 已接入 login、受保护路由、account/logout、user management entry、source/import/review/audit 页面，并有 browser smoke。                                 |
| Worker API shell      | Yes      | Yes        | Yes      | Partial  | No               | `apps/worker` 暴露 health/runtime primitives，并有最小 Cloudflare Vitest runtime smoke。                                                               |
| Auth                  | Yes      | Yes        | Partial  | Partial  | No               | Invite link、accept page、login、protected routing、account/logout、admin user/invitation management、reset page、session revocation 和 audit 已集成。 |
| RBAC                  | Yes      | Partial    | Partial  | Partial  | No               | `@qitu/rbac` 定义 owner/admin/reviewer/viewer；Worker 写路由检查权限并写 audit/security-event `rbac.denied`；多租户 scope 仍是未来工作。               |
| Files                 | Yes      | Yes        | Partial  | Partial  | No               | 鉴权上传会写 R2/D1/audit；source list API 和 upload UI 已接 Worker API。                                                                               |
| Jobs                  | Yes      | Partial    | Partial  | Partial  | No               | 上传创建 queued import job；handler/local drain 推进 processing/needs_review/failed；failed jobs 可列出、retry，并通过 `import_job_events` 追踪。      |
...
| Human review          | Yes      | Partial    | Partial  | Partial  | No               | Review/decision/commit/retry API 和 demo console 已存在；handler 与 browser smoke 覆盖 approve、commit、reject。                                       |
| Audit/security/alerts | Yes      | Partial    | Partial  | Partial  | No               | 敏感路径写 audit；login attempts、security events、import job events、alert events、list API 与 web timeline 已接入。                                  |
...
| AI advisory           | Yes      | Partial    | Partial  | Partial  | No               | 本地 deterministic import-review advisory artifacts、list/generate/confirm/dismiss routes、web panel、audit events 已接入。                            |
...
| Design system         | Yes      | Yes        | Yes      | Partial  | No               | 已实现 FOF 抽象后的 business-neutral workbench tokens、shell、surfaces、metrics、data states、timeline 与 review console composition。                 |
