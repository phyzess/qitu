...
| --------------------- | -------- | ---------- | -------- | -------- | ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| React app shell       | Yes      | Yes        | Yes      | Partial  | No               | 已接入 login、受保护路由、account/logout、持久化中英文选择、user management entry、source/import/review/audit 页面、role-aware write controls、source/job diagnostics、import-to-review selection memory，并有 browser smoke。 |
