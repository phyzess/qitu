...
| --------------------- | -------- | ---------- | -------- | -------- | ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| React app shell       | Yes      | Yes        | Yes      | Partial  | No               | API-backed login, protected routes, account/logout, user management entry, source/import/review/audit pages, and scripted browser smoke coverage.                                       |
...
| Auth                  | Yes      | Yes        | Partial  | Partial  | No               | Invite links, accept page, login, protected routing, account/logout, admin user/invitation management, reset page, session revocation, security events, and audit paths are integrated. |
| RBAC                  | Yes      | Partial    | Partial  | Partial  | No               | `@qitu/rbac` defines owner/admin/reviewer/viewer; Worker write routes enforce permissions and audit/security-event `rbac.denied`; multi-tenant scopes remain future work.               |
...
| Jobs                  | Yes      | Partial    | Partial  | Partial  | No               | Upload creates queued import job; handler/local drain advance processing/needs_review/failed; failed jobs can be listed, retried, and traced through `import_job_events`.               |
...
| Audit/security/alerts | Yes      | Partial    | Partial  | Partial  | No               | Sensitive paths write audit; login attempts, security events, import job events, alert events, list APIs, and web timelines exist for runtime visibility.                               |
...
| Design system         | Yes      | Yes        | Yes      | Partial  | No               | FOF-derived business-neutral workbench tokens, shell, surfaces, metrics, data states, timeline, and review console composition are implemented.                                         |
