...
### 2026-07-15：TypeScript 7 正式版基线

Decision:

用精确稳定版本 `typescript@7.0.2` 替换最初的 TypeScript 7 RC pin。

规则：

1. Root 与所有显式依赖 TypeScript 的 workspace package 都使用 `7.0.2`。
2. TypeScript 基线变化时，package manifests、lockfile、toolchain guards 与当前依赖文档必须同步更新。
3. 配置的 registry 必须同时为本地开发与 CI 提供 `typescript` wrapper 和所需的
   `@typescript/typescript-*` 平台包。
4. 2026-06-26 的 RC 条目保留为历史记录；active baseline 以本 decision 为准。

原因：

稳定版 compiler 已通过 workspace 验证路径；精确锁定正式版本能保持 install 可复现，也避免
starter 长期停留在 prerelease toolchain。
